...
Составные сообщения раскладываются на отдельные строки по нумерованным работам,
сотрудникам и уникальным обозначениям изделий. Если часы между работами или
изделиями не распределены однозначно, ProLOG не дублирует общий итог, а просит
пользователя указать часы по каждой строке во входящей очереди.
...
Администратор выполняет первоначальное сопоставление `MAX ID → сотрудник` в
окне привязок ProLOG. Имена профилей и телефоны не используются как надежное
доказательство личности. После сохранения привязки уже загруженные входящие
отчеты пересопоставляются без создания дублей.

Пути к базам ведутся в справочнике «Базы данных». Данные разделены по назначению:

- `prolog.sqlite3` — журнал работ, настройки, оплата и очередь WorkBot;
- `employees.sqlite3` — сотрудники;
- `objects.sqlite3` — объекты;
- `products.sqlite3` — изделия;
- `aliases.sqlite3` — подтверждённые варианты написания сотрудников, объектов,
  местонахождений, видов работ и изделий;
- `workbot.sqlite3` — исходные сообщения чат-бота.

Каждый существующий SQLite-файл можно выбрать из локальной или сетевой папки.
Компоненты ProLOG подключаются после перезапуска. При первом запуске новой версии
данные из прежней единой базы переносятся автоматически с сохранением ID.
